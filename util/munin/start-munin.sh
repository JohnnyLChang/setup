...
do
  NAME=`echo $NODE | cut -d ":" -f1`
  HOST=`echo $NODE | cut -d ":" -f2`
  COMM=`echo $NODE | cut -d ":" -f3`
  if [ ${#PORT} -eq 0 ]; then
      PORT=4949
  fi
  sudo -u munin munin-node-configure -snmp $HOST -snmpcommunity $COMM --shell | sh
   if ! grep "\[$NAME\]" /etc/munin/munin.conf ; then
    cat << EOF >> /etc/munin/munin.conf
[$NAME]
    address 127.0.0.1
    use_node_name no
